...
async function main() {
    ...
    if (!fs.existsSync(zkeyPath)) {
        ...
        console.error('  npx snarkjs groth16 setup build/main.r1cs ptau/pot15_final.ptau build/main_0000.zkey');
        console.error('  npx snarkjs zkey contribute build/main_0000.zkey build/main_final.zkey -e="entropy"');
        console.error('  npx snarkjs zkey export verificationkey build/main_final.zkey build/verification_key.json');
        process.exit(1);
    }
    ...
    console.log('1. Use the Rust contract\'s Poseidon implementation to compute test values');
    console.log('2. Export the computed commitment, nullifierHash, and stateRoot');
    ...
    console.log('  3. Run: npx snarkjs groth16 fullprove build/sample_input.json build/main_js/main.wasm build/main_final.zkey build/proof.json build/public.json');
    console.log('  4. Verify: npx snarkjs groth16 verify build/verification_key.json build/public.json build/proof.json');
}
